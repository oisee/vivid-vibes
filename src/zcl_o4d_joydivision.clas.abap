CLASS zcl_o4d_joydivision DEFINITION PUBLIC FINAL CREATE PUBLIC.
  PUBLIC SECTION.
...
CLASS zcl_o4d_joydivision IMPLEMENTATION.
  ...
  METHOD get_spectrum.
    ...
    lv_seed = CONV i( iv_time * 500 ).  " Slower sampling

    DO mv_num_bars TIMES.
      DATA(lv_i) = sy-index - 1.

      " Base wave - slower, smoother
  ...
  METHOD zif_o4d_effect~get_name. rv_name = 'joydivision'. ENDMETHOD.
  ...
  METHOD zif_o4d_effect~render_frame.
    ...
    " Only sample every 0.15 seconds (slower updates)
    ...
    APPEND VALUE zif_o4d_effect=>ty_text(
      x = c_width / 2 y = 25
      text = 'UNKNOWN PLEASURES' color = '#888' size = 10 align = 'center'
    ) TO rs_frame-texts.
