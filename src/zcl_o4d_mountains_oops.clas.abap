CLASS zcl_o4d_mountains_oops DEFINITION PUBLIC CREATE PUBLIC.
*======================================================================
...
CLASS ZCL_O4D_MOUNTAINS_OOPS IMPLEMENTATION.
  ...
  METHOD zif_o4d_effect~render_frame.
    ...
    DO 20 TIMES.
      ...
      DATA(ls_sky_rect) = VALUE zif_o4d_effect=>ty_rect(
        x = 0 y = ( sy-index - 1 ) * 20
        w = lc_w h = 21
        fill = |hsl({ lv_sky_hue }, 60%, { CONV i( lv_sky_l ) }%)| ).

      IF lv_phase = 2.
        " Correct phase: sky behind sun (rects_back → circles → rects)
        APPEND ls_sky_rect TO rs_frame-rects_back.
      ELSE.
        " Wrong phase: square sun is in rects anyway
        APPEND ls_sky_rect TO rs_frame-rects.
      ENDIF.
    ENDDO.
    ...
    ELSE.
      ...
      APPEND VALUE zif_o4d_effect=>ty_circle(
        ...
        radius = lv_sun_size * '0.7'
        fill = '#FFDD00'
      ) TO rs_frame-circles.

      APPEND VALUE zif_o4d_effect=>ty_circle(
        x = lv_sun_x y = lv_sun_y
        radius = lv_sun_size * '1.1'
        fill = '#FFAA0033'
      ) TO rs_frame-circles.
    ENDIF.
    ...
    ELSEIF lv_phase = 1.
      ...
      APPEND VALUE zif_o4d_effect=>ty_text(
        x = lc_w / 2 y = lc_h / 2
        text = 'OOPS!'
        color = '#FF8888' size = 48 align = 'center'
      ) TO rs_frame-texts.
